...
(function() {
    ...
    function renderMainUI() {
        ...
        const isDetailPage = window.location.href.includes('first_page.php') || document.querySelectorAll('button').length > 10;
        
        if (STATE.activePhase === 'IDLE' || !isDetailPage) {
            contentHTML = `
    ...
    function parseExcel(file, projType, callback) {
        if (typeof XLSX === 'undefined') {
            showNotice('Hata', 'Excel kütüphanesi (XLSX) yüklenemedi. Lütfen internet bağlantınızı kontrol edin veya sayfayı yenileyin.');
            return;
        }
        ...
        parseExcel(fileInput.files[0], projType, (jsonData) => {
            ...
            for(let t of trs) {
                if (cleanString(t.innerText).includes(cleanString(searchText))) {
                    ...
                    if (btn && lowerTR(btn.innerText).includes('takibe gir')) {
                        foundButton = btn;
                        break;
                    }
                }
    ...
    async function processDetailForms() {
        ...
        const db = STATE.data;
        if (!db) {
            logMessage("Hata: Excel verisi bulunamadı. Lütfen Excel dosyasını tekrar yükleyin.");
        ...
        for (let btn of buttons) {
            const btnText = lowerTR(btn.innerText).trim();
            if (btnText === 'kaydet' || btnText === 'güncelle') {
                let container = btn.closest('div.card') || btn.closest('div.panel') || btn.closest('.box') || btn.parentElement.parentElement;
        ...
        for (let i = 0; i < blocks.length; i++) {
            ...
            if (!matchHeader || !matchHeader[1]) continue;
            ...
            if (STATE.processed.includes(cleanDomTitle)) continue;

            // Sitede zaten Tamamlandı mı?
            const selects = block.querySelectorAll('select');
            if (selects.length > 0) {
                const currentStatus = selects[0].options[selects[0].selectedIndex].text.trim();
                if (cleanString(currentStatus) === 'tamamlandi') {
                    logMessage(`[Atlandı] Sitede zaten 'Tamamlandı': ${domTitle}`);
            ...
            // Find in Excel Data
            const excIndex = db.findIndex(r => cleanString(r[colName] || r['Faaliyet Adı']) === cleanDomTitle || (r[colName] && cleanString(r[colName]).includes(cleanDomTitle)));
            ...
            matchedExcelIndices.add(excIndex);
            const excItem = db[excIndex];
        ...
        // Sayfadaki tüm görünür eylemler bitti. Şimdi Excel'de olup sayfada olmayanları kontrol et.
        const missingCompletedItems = db.filter((r, idx) => {
            const durumRaw = (r['DURUM'] || r['Durum'] || Object.values(r)[6] || '').toString();
            const name = r[colName] || r['Faaliyet Adı'] || '';
            return cleanString(durumRaw) === 'tamamlandi' && !matchedExcelIndices.has(idx) && !STATE.processed.includes(cleanString(name));
        });

        if (missingCompletedItems.length > 0) {
            logMessage(`${missingCompletedItems.length} adet sayfada görünmeyen ama Excel'de 'Tamamlandı' olan eylem var.`);
            for (const item of missingCompletedItems) {
                const itemName = item[colName] || item['Faaliyet Adı'] || 'İsimsiz Eylem';
                const cleanItemName = cleanString(itemName);
                let choice = 'skip';

                await new Promise(resolve => {
                    showModal(`Excel'de 'Tamamlandı' görünüyor ancak sayfada bulunamadı. İşleme (log kaydı) alınsın mı?`, itemName,
                        () => { choice = 'process'; resolve(); },
                        () => { choice = 'skip'; resolve(); },
                        () => { choice = 'cancel'; resolve(); }
                    );
                });
                ...
                const p = STATE.processed;
                p.push(cleanItemName);
                STATE.processed = p;

                if (choice === 'process') {
                    logMessage(`[Onaylandı] ${itemName} (Sitede bulunamadı ama Excel'de tamamlandı)`);
                } else {
                    logMessage(`[Atlandı] ${itemName} (Sitede bulunamadı)`);
                }
            }
        }
    ...
    function setSelectByText(selectElem, text) {
        ...
        const lowerText = lowerTR(text).trim();
        for (let opt of selectElem.options) {
            const optLower = lowerTR(opt.innerText).trim();
            if (optLower === lowerText || optLower.includes(lowerText)) {
    ...
    function fillFormElements(block, excItem, durum, isTamamlandi) {
        ...
        block.querySelectorAll('input[type="text"], input[type="date"]').forEach(inp => {
            ...
            const rawContext = lowerTR(inp.outerHTML + parentTexts + labelText + inpAttrs);
            
            // Eğer "plana göre" yazıyorsa bu alan MEB'in sabit alanıdır, dokunma!
            if (rawContext.includes('planagore') || rawContext.includes('sabit')) {
                return;
            }
    ...
    // SPA support: Re-render if panel is missing (navigation without reload)
    setInterval(() => {
        if (!document.getElementById('proiz-bot-panel')) {
            renderMainUI();
        }
    }, 3000);
