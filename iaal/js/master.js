document.addEventListener('DOMContentLoaded', () => {
    ...
    if (masterForm) {
        masterForm.addEventListener('submit', async (e) => {
            ...
            if (isNewSchool) {
                // Fiziksel klasör kopyalama denemesi
                ...
                try {
                    ...
                    console.warn("Fiziksel klasör oluşturulamadı (PHP desteklenmiyor olabilir):", e);
                }

                if (folderCreated) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Fiziksel Okul Klasörü Oluşturuldu! 🎉',
                        html: `<b>${schoolNameToUse}</b> için tamamen bağımsız yeni bir klasör oluşturuldu.<br><br>` + 
                              `<div style="text-align: left; background: #f8fafc; padding: 15px; border-radius: 10px; margin-top: 15px;">` +
                              `<strong style="color:var(--primary);">👨‍🏫 İdareci ve Öğretmen Girişi:</strong><br>` +
                    ...
                    const studentLink = window.location.origin + '/ogrenci.html?school=' + finalUsername;
                    const teacherLink = window.location.origin + '/index.html';
                    
                    Swal.fire({
                        icon: 'success',
                        title: 'Okul Başarıyla Oluşturuldu! 🎉',
                        html: `<b>${schoolNameToUse}</b> sisteme eklendi.<br><br>` + 
                              `<span style="color:var(--danger); font-size:0.85rem;">Not: Sunucunuzda PHP çalışmadığı için (Örn: Vercel) fiziksel klasör kopyalanamadı. Ancak aşağıdaki evrensel linkleri kullanarak okulu bağımsız olarak yönetebilirsiniz:</span><br><br>` +
                              `<div style="text-align: left; background: #f8fafc; padding: 15px; border-radius: 10px; margin-top: 15px;">` +
                              `<strong style="color:var(--primary);">👨‍🏫 Öğretmen/İdareci Giriş Linki:</strong><br>` +
                              `<a href="${teacherLink}" target="_blank" style="color: #4f46e5; text-decoration: underline; word-break: break-all;">${teacherLink}</a><br><br>` +
                              `<strong style="color:var(--secondary);">🎓 Öğrenci Giriş Linki:</strong><br>` +
                              `<a href="${studentLink}" target="_blank" style="color: #10b981; text-decoration: underline; word-break: break-all;">${studentLink}</a>` +
                              `</div>`,
                        confirmButtonText: 'Tamam'
                    });
                }
