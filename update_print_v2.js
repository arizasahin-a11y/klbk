...
const filePath = path.join('a:', 'TOOLS', 'kodlama', 'km', 'KLBK FRVR', 'h6t3y9w1');
let content = fs.readFileSync(filePath, 'utf8');

// 1. Remove the print button from exam-header
...
content = content.replace(printToolsHeaderRegex, '');

// 2. Add the original printModesHtml back to details-panel
...
content = content.replace(detailsPanelEndStr, replacementDetails);

// 3. Replace window.printSession and executePrintSession blocks completely
const scriptStartIdx = content.indexOf('window.printSession = async function (id) {');
const scriptEndIdx = content.lastIndexOf('        };'); // Last line before </script> of that block
const scriptEndFullIdx = content.indexOf('</script>', scriptEndIdx);

if (scriptStartIdx !== -1 && scriptEndIdx !== -1) {
    const beforeBlock = content.substring(0, scriptStartIdx);
    const afterBlock = content.substring(scriptEndFullIdx - 9); // back to '        }\n    </script>' roughly

    const newFns = `        window.printSession = async function (id) {
            ...
            let checkboxesHtml = entities.map(entity => \`
                <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; padding: 5px;">
                    ...
                    <span style="font-size: 1rem; font-weight: 600; color: #1e293b;">\${entity}</span>
                </label>
            \`).join('');
            ...
            Swal.fire({
                ...
                html: \`
                    <div style="text-align: left; padding: 10px; margin-bottom: 10px;">
                        <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; margin-bottom: 15px; border-bottom: 1px solid #cbd5e1; padding-bottom: 10px;">
                            <input type="checkbox" id="selectAllEntities" checked style="width: 18px; height: 18px; cursor: pointer; accent-color: var(--primary);" onchange="document.querySelectorAll('input[name=printEntity]').forEach(cb => cb.checked = this.checked)">
                            <span style="font-size: 1rem; font-weight: 800; color: var(--primary);">Tümünü Seç</span>
                        </label>
                        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(100px, 1fr)); gap: 10px; max-height: 300px; overflow-y: auto; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; padding: 10px;">
                            \${checkboxesHtml}
                        </div>
                    </div>
                \`,
        ...
        async function executePrintSession(id, mode, filterArray) {
            ...
            const pageCss = \`
                ...
                /* Explicitly assign page rules for modern browsers */
    ...
    fs.writeFileSync(filePath, beforeBlock + newFns + afterBlock, 'utf8');
    console.log("Complete Update Finished");
} else {
    console.error("Could not find start/end block");
}
