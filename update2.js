...
// 1. Re-apply loadSchoolSettings
const loadSchoolSettingsTarget = `        setVal('gradeLevels', school.gradeLevels ? school.gradeLevels.join(', ') : '');
        setVal('schoolSubjects', school.subjects ? school.subjects.join(', ') : '');`;
const loadSchoolSettingsReplacement = loadSchoolSettingsTarget + `\n        setVal('pdfHeaderDesign', school.pdfHeaderDesign || '1');`;
content = content.replace(loadSchoolSettingsTarget, loadSchoolSettingsReplacement);

// 2. Re-apply saveSchoolSettings
const saveSchoolSettingsTarget = `            dailyLessons: dailyLessons,
            lessonTimes: lessonTimes
        };

        DataManager.saveSchoolSettings(settings);`;
const saveSchoolSettingsReplacement = `            dailyLessons: dailyLessons,
            lessonTimes: lessonTimes,
            pdfHeaderDesign: document.getElementById('pdfHeaderDesign') ? document.getElementById('pdfHeaderDesign').value : '1'
        };

        DataManager.saveSchoolSettings(settings);`;
content = content.replace(saveSchoolSettingsTarget, saveSchoolSettingsReplacement);

// 3. Replace the canvas block
const blockStart = `                // ONLY DRAW ON PAGE 1
                if (i === 0) {
                    // --- PERFECT REPRODUCTION HEADER ---`;

const blockEnd = `                            const lx = ix + (leftW - logoDim) / 2;
...
const idx1 = content.indexOf(blockStart);
const idx2 = content.indexOf(blockEnd);

if (idx1 === -1 || idx2 === -1) {
    console.error("Canvas block markers not found");
    process.exit(1);
}

const endPos = idx2 + blockEnd.length;

const newCanvasLogic = `                // ONLY DRAW ON PAGE 1
                if (i === 0) {
...
content = content.substring(0, idx1) + newCanvasLogic + content.substring(endPos);

fs.writeFileSync(uiJsPath, content, 'utf8');
console.log('Update complete via substring block replacement');
